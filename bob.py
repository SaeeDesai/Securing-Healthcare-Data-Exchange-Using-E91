...
with open("final_key_bob.txt", "rb") as key_file:
    aes_key = key_file.read(16)

def delete_key_file():
    try:
        os.remove("final_key_bob.txt")
...
def send_message_to_alice(data, key, message_listbox, recipient):
...
def send_message_to_doctor(message_entry, aes_key, message_listbox, recipient):
    ...
    if message:
        ...
        send_message_to_alice(message, aes_key, message_listbox, recipient)
        message_entry.delete(0, tk.END)

def send_message_to_nurse(message_entry, aes_key, message_listbox, recipient):
    print("Sending message to nurse...\n")
    message = message_entry.get()
    if message:
        # Handle regular message sending
...
def create_gui(aes_key):
    root = tk.Tk()
    root.title("Bob's Secure Message Exchange")
    ...
    doctor_button = tk.Button(message_frame, text="Send to Doctor Alice", command=partial(send_message_to_doctor, message_entry, aes_key, message_listbox, "Doctor"))
            ...
            while True:
                ...
                if received_message.startswith(b"MSG:"):
                    ...
                    if len(message_lines) > 1:
                        ...
                        msg_str = message
                        message_listbox.insert(tk.END, f"[{received_timestamp}] Alice: {msg_str}")
                        message_listbox.see(tk.END)
                    else:
                    ...
                    message_listbox.insert(tk.END, "File saved as 'file_from_alice.txt'.")
                    message_listbox.see(tk.END)
